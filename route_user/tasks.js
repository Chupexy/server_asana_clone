...
router.post('/set_privacy', async(req, res) =>{
    const {token, privacy_status} = req.body
    if(!token || !privacy_status)
        return res.status(400).send({status: 'error', msg:'All fields must be filled'});
...
router.post('/set_priority', async (req,res) =>{
    ...
    try {
        ...
        const task = await Project.tasks.findone({task_id: task_id}, {priority: 1}).lean()
        if(!task)
            return res.status(200).send({status: 'ok', msg: 'No project found'})

        task = await Project.tasks.findOneAndUpdate({task_id: task_id}, {priority: priority}).lean()
